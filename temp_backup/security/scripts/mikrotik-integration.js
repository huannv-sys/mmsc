...
const RouterOSAPI =
  nodeRouterOS.RouterOSAPI ||
  (nodeRouterOS.default && nodeRouterOS.default.RouterOSAPI);
const RosApiPool =
  nodeRouterOS.RosApiPool ||
  (nodeRouterOS.default && nodeRouterOS.default.RosApiPool);
...
// Biến toàn cục
let isRunning = false;
const rosClients = new Map(); // deviceId -> ROS client
...
async function connectToDevice(device) {
  ...
  try {
    ...
    console.log(
      `Đã kết nối thành công đến thiết bị MikroTik ${device.name} (${ipAddress})`,
    );

    return connection;
  } catch (error) {
    console.error(
      `Lỗi khi kết nối đến thiết bị MikroTik ${device.name} (${ipAddress}):`,
      error.message || error,
    );
    return null;
  }
...
async function ensureAddressList(conn, listName) {
  try {
    ...
    if (!conn || typeof conn.query !== "function") {
      console.error(
        `Kết nối không hợp lệ khi đảm bảo address list ${listName}`,
      );
      return;
    }
    ...
    try {
      ...
      const lists = await conn
        .query("/ip/firewall/address-list/print")
        .where({
          list: listName,
        })
        .exec();
      ...
      if (lists.length === 0) {
        ...
        if (items.length > 0) {
          await conn
            .query("/ip/firewall/address-list/remove")
            .equal({
              ".id": items[0][".id"],
            })
            .exec();
    ...
    console.error(
      `Lỗi khi đảm bảo address list ${listName}:`,
      error.message || error,
    );
  }
}
...
async function addToBlocklist(conn, ip, timeout, comment) {
  try {
    ...
    if (!conn || typeof conn.query !== "function") {
      console.error(
        `Kết nối không hợp lệ khi thêm IP ${ip} vào danh sách chặn`,
      );
      return false;
    }
    ...
    try {
      ...
      if (items && items.length > 0) {
        ...
        await conn
          .query("/ip/firewall/address-list/set")
          .equal({
            ".id": items[0][".id"],
            timeout,
            comment,
    ...
    console.error(
      `Lỗi khi thêm IP ${ip} vào danh sách chặn:`,
      error.message || error,
    );
    return false;
  }
...
async function removeFromBlocklist(conn, ip) {
  try {
    ...
    if (!conn || typeof conn.query !== "function") {
      console.error(
        `Kết nối không hợp lệ khi xóa IP ${ip} khỏi danh sách chặn`,
      );
      return false;
    }
    ...
    console.error(
      `Lỗi khi xóa IP ${ip} khỏi danh sách chặn:`,
      error.message || error,
    );
    return false;
  }
...
async function getBlockedIPs(conn) {
  try {
    ...
    try {
      ...
      if (!addressLists || addressLists.length === 0) {
        console.log(
          `Không tìm thấy danh sách chặn '${BLOCK_LIST_NAME}', trả về danh sách trống`,
        );
        return [];
      }
...
async function cleanupExpiredIPs(conn) {
  try {
    ...
    try {
      ...
      console.error(
        "Lỗi khi truy vấn danh sách địa chỉ trong quá trình dọn dẹp:",
        queryError,
      );
    }
  } catch (error) {
...
async function processAlerts() {
  try {
    ...
    console.log(
      `Tìm thấy ${securityAlerts.length} cảnh báo bảo mật mới cần xử lý`,
    );
    ...
    if (activeDevices.length === 0) {
      console.log(
        "Không có thiết bị MikroTik nào đang hoạt động để xử lý cảnh báo",
      );
      return;
    }
...
function startIntegration() {
  ...
  console.log(
    `Đã bắt đầu tích hợp, kiểm tra cảnh báo mỗi ${CHECK_INTERVAL / 1000} giây`,
  );
}
