...
const blacklistedIPs = new Map(); // IP -> {count, timestamp}
...
async function saveAlertsToDB(suricataAlerts) {
  for (const alert of suricataAlerts) {
    try {
      ...
      console.log(
        `Phát hiện cảnh báo mới: ${alertMessage} (IP nguồn: ${srcIP}, IP đích: ${dstIP})`,
      );
...
async function blockIPOnMikroTik(ip) {
  try {
    ...
    if (onlineDevices.length === 0) {
      console.log(
        "Không có thiết bị MikroTik đang hoạt động để thêm quy tắc chặn.",
      );
      return;
    }
    ...
    for (const device of onlineDevices) {
      try {
        ...
        console.log(
          `Thêm quy tắc chặn IP ${ip} trên thiết bị ${device.name} (${device.ipAddress})`,
        );
        ...
        console.error(
          `Lỗi khi thêm quy tắc chặn trên thiết bị ${device.name}:`,
          deviceError,
        );
      }
    }
...
function startMonitoring() {
  ...
  console.log(
    `Hệ thống giám sát đã khởi động, kiểm tra cảnh báo mỗi ${CHECK_INTERVAL / 1000} giây.`,
  );
}
