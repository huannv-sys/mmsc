...
const mikrotikIntegrationPath = path.resolve(
  SECURITY_DIR,
  "scripts/mikrotik-integration.js",
);
...
export function setupSecurityRoutes(router: Router) {
    ...
    try {
      ...
      if (fs.existsSync(path.join(RULES_DIR, "mikrotik.rules"))) {
        const rulesContent = fs.readFileSync(
          path.join(RULES_DIR, "mikrotik.rules"),
          "utf8",
        );
        ruleCount = rulesContent
          .split("\n")
      ...
      try {
        ...
        alertCount = alerts.filter((alert) =>
      ...
      res.status(500).json({
        error: "Lỗi khi kiểm tra trạng thái Suricata",
  ...
  router.post("/security/start", (_req: Request, res: Response) => {
    try {
      if (isMonitoring) {
        return res
          .status(400)
          .json({ error: "Hệ thống giám sát đã đang chạy" });
      }
      ...
      if (securityMonitor) {
        ...
        const networkInterface =
          execSync("ip route | grep default | awk '{print $5}'")
            .toString()
            .trim() || "eth0";
      ...
      res.status(500).json({
        error: "Lỗi khi bắt đầu giám sát bảo mật",
        details: error.message,
      });
    }
  });
  ...
  router.post("/security/stop", (_req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        error: "Lỗi khi dừng giám sát bảo mật",
        details: error.message,
      });
    }
  });
  ...
  router.get("/security/rules", async (_req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Lỗi khi lấy quy tắc bảo mật", details: error.message });
    }
  });
  ...
  router.post("/security/rules", async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        error: "Lỗi khi thêm quy tắc bảo mật",
        details: error.message,
      });
    }
  });
  ...
  router.delete("/security/rules/:sid", async (req: Request, res: Response) => {
    try {
      ...
      const updatedRules = rulesContent
        .split("\n")
      ...
      res
        .status(500)
        .json({ error: "Lỗi khi xóa quy tắc bảo mật", details: error.message });
    }
  });
  ...
  router.get("/security/alerts", async (req: Request, res: Response) => {
    try {
      ...
      const securityAlerts = alerts.filter(
        (alert) =>
          alert.message.startsWith("[Suricata]") &&
          !alert.message.includes("1000000"),
      );
      ...
      res.status(500).json({
        error: "Lỗi khi lấy cảnh báo bảo mật",
        details: error.message,
      });
    }
  });
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Lỗi khi xác nhận cảnh báo", details: error.message });
      }
    },
  );
  ...
  router.post(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        res.status(500).json({
          error: "Lỗi khi xác nhận tất cả cảnh báo",
          details: error.message,
        });
      }
    },
    ...
    try {
      ...
      if (!mikrotikIntegration) {
        ...
        if (!mikrotikIntegration) {
          return res
            .status(500)
            .json({ error: "Mô-đun tích hợp MikroTik không thể tải" });
        }
      }
      ...
      if (!connection) {
        return res
          .status(500)
          .json({ error: "Không thể kết nối đến thiết bị MikroTik" });
      }
      ...
      try {
        ...
        if (typeof connection.query !== "function") {
          console.error(
            "Lỗi khi lấy danh sách IP bị chặn: Kết nối không hợp lệ",
          );
          return res.json({ blockedIPs: [] });
        }
      ...
      res.status(500).json({
        error: "Lỗi khi lấy danh sách IP bị chặn",
        details: error.message,
      });
    }
  });
  ...
  router.post("/security/block-ip", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        res.status(500).json({
          error: "Không thể chặn IP",
          details: blockError.message || blockError,
        });
      }
    } catch (error) {
      ...
      res
        .status(500)
        .json({ error: "Lỗi khi chặn IP", details: error.message });
    }
  });
  ...
  router.post("/security/unblock-ip", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const success = await mikrotikIntegration.removeFromBlocklist(
          connection,
          ip,
        );
        ...
        res.status(500).json({
          error: "Không thể gỡ bỏ chặn IP",
          details: unblockError.message || unblockError,
        });
      }
    } catch (error) {
      ...
      res
        .status(500)
        .json({ error: "Lỗi khi gỡ bỏ chặn IP", details: error.message });
    }
  });
  ...
  router.get("/security/logs", (_req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        error: "Lỗi khi lấy nhật ký Suricata",
        details: error.message,
      });
    }
  });
  ...
  router.get("/security/logs/:filename", (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        error: "Lỗi khi lấy nội dung nhật ký",
        details: error.message,
      });
    }
  });
    ...
    async (_req: Request, res: Response) => {
      try {
        if (!mikrotikIntegration) {
          return res
            .status(500)
            .json({ error: "Mô-đun tích hợp MikroTik chưa được tải" });
        }
        ...
        res
          .status(500)
          .json({ error: "Lỗi khi xử lý cảnh báo", details: error.message });
      }
    },
  );
