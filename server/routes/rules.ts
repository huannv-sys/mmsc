...
rulesRouter.get("/devices/:id/rules", async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const [firewallRules, natRules, mangleRules, queueData, routingData] =
        await Promise.all([
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          (innerError as Error).message.includes(
...
rulesRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        const newRule = await mikrotikRulesService.createFirewallRuleByDeviceId(
          deviceId,
          ruleData,
        );
...
rulesRouter.put(
  "/devices/:id/rules/:type/:ruleId/toggle",
  async (req: Request, res: Response) => {
    try {
      const deviceId = Number(req.params.id);
      ...
      try {
        ...
        switch (ruleType) {
          case "firewall":
            result = await mikrotikRulesService.toggleFirewallRuleByDeviceId(
              deviceId,
              ruleId,
              disabled,
            );
            break;
          case "nat":
            result = await mikrotikRulesService.toggleNatRuleByDeviceId(
              deviceId,
              ruleId,
              disabled,
            );
            break;
          case "mangle":
            result = await mikrotikRulesService.toggleMangleRuleByDeviceId(
              deviceId,
              ruleId,
              disabled,
            );
            break;
          case "queue":
            result = await mikrotikRulesService.toggleQueueRuleByDeviceId(
              deviceId,
              ruleId,
              disabled,
            );
            break;
          case "routing":
            result = await mikrotikRulesService.toggleRoutingRuleByDeviceId(
              deviceId,
              ruleId,
              disabled,
            );
