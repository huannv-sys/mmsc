    ...
    try {
      ...
      try {
        ...
        if (!Array.isArray(capsmanAPData)) {
          console.warn('Invalid CAPsMAN AP data format, fallback to remote-cap search');
          capsmanAPData = [];
        } else {
      ...
      if (capsmanAPData.length === 0) {
        try {
          ...
          if (Array.isArray(interfaces)) {
            ...
            console.log(`Found ${capInterfaces.length} potential CAP interfaces`);
      ...
      if (capsmanAPData.length === 0) {
        ...
        console.log(`Found total of ${capsmanAPData.length} CAPsMAN access points`);
      }
    ...
    try {
      ...
      try {
        ...
        if (!Array.isArray(registrations)) {
          console.warn(`No valid registration data for AP ${ap.name} from registration-table`);
          registrations = [];
        }
      } catch (registrationError) {
        console.warn(`Could not get registration table for AP ${ap.name}:`, registrationError);
        registrations = [];
      }
      ...
      if (registrations.length === 0) {
        try {
          ...
          registrations = await client.executeCommand('/interface/wireless/registration-table/print', [
            { '?interface': ap.name }
          ]);
          
          if (Array.isArray(registrations) && registrations.length > 0) {
            console.log(`Found ${registrations.length} clients from wireless registration for AP ${ap.name}`);
          } else {
            registrations = [];
          }
        } catch (wirelessRegError) {
          console.warn(`Could not get wireless registration table for AP ${ap.name}:`, wirelessRegError);
        }
      }
