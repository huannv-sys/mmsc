...
export class LogAnalyzerService {
  ...
  async analyzeTrafficLogs(
    ...
    try {
      ...
      console.error(
        `Error analyzing traffic logs for device ${deviceId}:`,
        error,
      );
      return {
        success: false,
  ...
  private processTrafficLogs(
    ...
    const protocolStats: Record<
      string,
      { connections: number; bytes: number }
    > = {};

    // Thống kê theo IP nguồn
    const sourceIpStats: Record<
      string,
      { connections: number; bytes: number }
    > = {};
    ...
    for (const log of logs) {
      ...
      if (log.time && log.date) {
        const dateStr = log.date.replace(
          /([0-9]+)\/([0-9]+)\/([0-9]+)/,
          "$3-$1-$2",
        );
      ...
      if (log.message) {
        // Kết nối TCP/UDP
        const connMatch = log.message.match(
          /([a-z]+),([0-9a-f\.:]+):([0-9]+)->([0-9a-f\.:]+):([0-9]+)/i,
        );
        if (connMatch) {
        ...
        // Kết nối ICMP
        const icmpMatch = log.message.match(
          /ICMP,([0-9a-f\.:]+)->([0-9a-f\.:]+)/i,
        );
        if (icmpMatch) {
        ...
        const interfaceMatch =
          log.message.match(/in:([a-z0-9-]+)/i) ||
          log.message.match(/out:([a-z0-9-]+)/i);
        if (interfaceMatch) {
    ...
    const protocolStatsList = Object.entries(protocolStats)
      ...
      .map(([protocol, stats]) => ({
        ...
        bytesFormatted: this.formatBytes(stats.bytes),
        percentage:
          summary.totalBytes > 0 ? (stats.bytes / summary.totalBytes) * 100 : 0,
      }));
    ...
    const formattedSummary = {
      ...
      averageBandwidthMbps: this.calculateAverageBandwidth(
        summary.totalBytes,
...
export function initLogAnalyzerService(
