...
class TrafficCollectorService {
  ...
  private async collectMikrotikTraffic(device: any): Promise<{
    ...
    try {
      ...
      if (!mikrotik) {
        console.log(
          "Failed to connect to MikroTik device, using generic collector",
        );
      ...
      const stats = await mikrotik.execute(
        `/interface/monitor-traffic once=yes numbers=${mainInterface.name}`,
      );
      ...
      console.error(
        `Error collecting MikroTik traffic for device ID ${device.id}:`,
        error,
      );
