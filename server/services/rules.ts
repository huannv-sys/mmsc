...
export class RulesService {
  ...
  async getMangleRules(deviceId: number): Promise<RuleResult> {
    try {
      ...
      // Lấy mangle rules
      const mangleRules = await client.executeCommand('/ip/firewall/mangle/print');
  ...
  async getQueueRules(deviceId: number): Promise<RuleResult> {
    try {
      ...
      const simpleQueueRules = await client.executeCommand('/queue/simple/print');
  ...
  async getAllRules(deviceId: number): Promise<RuleResult> {
    try {
      ...
      // Lấy các loại rules
      const [firewallResult, natResult, mangleResult, queueResult, routingResult] = await Promise.all([
      ...
      if (!firewallResult.success || !natResult.success || !mangleResult.success || 
          !queueResult.success || !routingResult.success) {
        return {
          success: false,
