...
import {
  initLogAnalyzerService,
  getLogAnalyzerService,
...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      if (req.query.check === "true") {
        ...
        for (const device of secureDevices) {
          if (device.ipAddress) {
            ...
            const isOnline =
              originalDevice &&
              originalDevice.username &&
              originalDevice.password
                ? await mikrotikService.checkDeviceOnline(
                    device.ipAddress,
                    originalDevice.username,
                    originalDevice.password,
                  )
            ...
            if (device.isOnline !== isOnline) {
              console.log(
                `Trạng thái thiết bị ${device.name} (${device.ipAddress}) đã thay đổi: ${device.isOnline} -> ${isOnline}`,
              );
      ...
      if (req.query.includePPPConnections === "true") {
        try {
          const pppConnections =
            await mikrotikService.getLTPPConnections(deviceId);
  ...
  router.post("/interfaces/:id/toggle", async (req: Request, res: Response) => {
    try {
      ...
      const result = await mikrotikService.toggleInterface(
        deviceId,
        interfaceId,
        enable,
      );
  ...
  router.get("/devices/:id/wireless", async (req: Request, res: Response) => {
    try {
      ...
      const wirelessInterfaces =
        await wirelessService.getWirelessInterfaces(deviceId);
  ...
  router.get("/wireless/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (!wirelessInterface) {
        return res
          .status(404)
          .json({ message: "Wireless interface not found" });
      }
  ...
  async function createCapsmanAPsFromInterfaces(deviceId: number) {
    try {
      ...
      try {
        ...
        console.log(
          `Đang xóa ${existingAPs.length} APs hiện có trong cơ sở dữ liệu`,
        );
      ...
      const capInterfaces = interfaces.filter((iface) => {
        ...
        const typeMatches =
          !iface.type || iface.type === "wlan" || iface.type === "cap";
      ...
      console.log(
        `Các giao diện tiềm năng: ${capInterfaces.map((i) => i.name).join(", ")}`,
      );

      // Nếu không có interfaces phù hợp, thử với tất cả interfaces wireless
      if (capInterfaces.length === 0) {
        console.log(
          "Không tìm thấy interfaces phù hợp theo tên, tìm kiếm theo type...",
        );

        // Lấy danh sách wireless interfaces
        try {
          const wirelessInterfaces = await client.executeCommand(
          ...
          if (
            ...
            console.log(
              `Tìm thấy ${wirelessInterfaces.length} giao diện wireless`,
            );
  ...
  router.get("/devices/:id/capsman", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Tìm thấy ${capsmanAPs.length} CAPsMan APs trong cơ sở dữ liệu cho thiết bị ${deviceId}`,
      );
      ...
      const hasCustomNetworks = capsmanAPs.some((ap) =>
        customNetworks.includes(ap.name),
      );
      ...
      if (!hasCustomNetworks && capsmanAPs.length > 0) {
        ...
        for (const config of customWifiConfigs) {
          ...
          try {
            ...
            console.error(
              `Lỗi khi tạo AP tùy chỉnh ${config.name}:`,
              createError,
            );
          }
        }
      ...
      if (capsmanAPs.length === 0) {
        console.log(
          `Không có AP nào, tạo từ interfaces cho thiết bị ${deviceId}...`,
        );
        try {
          ...
          try {
            ...
            capsmanAPs = await capsmanService.getCapsmanAPs(deviceId);
            console.log(
              `Đã thu thập và tìm thấy ${capsmanAPs.length} CAPsMan APs`,
            );
          } catch (collectError) {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!device.hasCAPsMAN) {
          return res
            .status(400)
            .json({ message: "Device does not support CAPsMAN" });
        }

        console.log(
          `Bắt đầu làm mới dữ liệu CAPsMAN cho thiết bị ${deviceId}...`,
        );
        ...
        if (capInterfaces.length > 0) {
          for (const iface of capInterfaces) {
            ...
            const isRunning =
              iface.running === true ||
              (typeof iface.running === "string" && iface.running === "true");
        ...
        if (capsmanAPs.length > 0) {
          ...
          try {
            console.log(
              "Thử tạo APs thủ công từ thông tin cấu hình CAPsMAN...",
            );
            ...
            console.log(
              `Sử dụng ${wifiConfigs.length} cấu hình mạng Wi-Fi mặc định`,
            );
            ...
            for (const config of wifiConfigs) {
              ...
              try {
                ...
                console.error(
                  `Lỗi khi tạo AP từ cấu hình ${config.name}:`,
                  createError,
                );
              }
            }
            ...
            console.error(
              "Lỗi khi thử phương pháp thay thế:",
              alternativeError,
            );
  ...
  router.post("/alerts", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid alert data", errors: error.errors });
      }
  ...
  router.post("/devices/:id/refresh", async (req: Request, res: Response) => {
    try {
      ...
      if (!success) {
        return res
          .status(500)
          .json({ message: "Failed to collect device metrics" });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid interval", errors: error.errors });
        }
  ...
  router.post(
    "/scheduler/max-concurrent-devices",
    async (req: Request, res: Response) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid device count", errors: error.errors });
        }
        res
          .status(500)
          .json({ message: "Failed to update max concurrent devices" });
      }
    },
  );
  ...
  router.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to get device polling status" });
      }
    },
  );
  ...
  router.post("/devices/discover", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid subnet format", errors: error.errors });
      }
      console.error("Error during network discovery:", error);
      return res
        .status(500)
        .json({ message: "Failed to discover devices on network" });
    }
  });
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch RouterOS information" });
      }
    },
  );
  ...
  router.get("/clients/:id", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        console.error(
          `Error getting device details for ID ${deviceId}:`,
          detailsError,
        );
      }
    ...
    try {
      ...
      try {
        ...
        const monitoringMethods =
          deviceClassifierService.getMonitoringMethodsForRole(role);
        ...
        console.error(
          `Error classifying device ID ${deviceId}:`,
          classifyError,
        );
  ...
  router.post("/clients/add-device", async (req: Request, res: Response) => {
    try {
      ...
      const added = await clientManagementService.addDeviceToMonitoring(
        device as NetworkDeviceDetails,
      );
  ...
  router.post("/clients/scan", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `Thực hiện quét mạng với routerId = ${routerId}, subnet = ${subnet || "auto"}`,
      );
      ...
      try {
        ...
        console.log(
          `Kiểm tra kết nối đến router ${router.name} (${router.ipAddress})`,
        );
        ...
        if (!connected) {
          return res.status(400).json({
            success: false,
            message:
              "Could not connect to router. Please check router credentials.",
          });
        }
        ...
        console.log(
          `Tìm thấy ${arpEntries.length} bản ghi ARP từ router ${router.name}`,
        );

        // Lấy thông tin DHCP leases
        const dhcpLeases = await mikrotikService.getDhcpLeases(routerId);
        console.log(
          `Tìm thấy ${dhcpLeases.length} bản ghi DHCP từ router ${router.name}`,
        );
      ...
      if (devices.length > 0) {
        ...
        for (const device of devices) {
          const added =
            await clientManagementService.addDeviceToMonitoring(device);
          if (added) {
        ...
        console.log(
          "Không tìm thấy thiết bị từ phương pháp quét thông thường, thử phương pháp quét trực tiếp",
        );

        // Lấy thông tin neighbor trực tiếp từ router
        const directDevices = await mikrotikService.getNetworkNeighbors({
          id: routerId,
        });
        console.log(
          `Phát hiện ${directDevices.length} thiết bị bằng phương pháp trực tiếp`,
        );
        ...
        if (directDevices.length > 0) {
          ...
          for (const device of directDevices) {
            ...
            const added =
              await clientManagementService.addDeviceToMonitoring(
                networkDevice,
              );
            if (added) {
              addedDevices.push(added);
  ...
  router.post("/clients/:id/refresh", async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        `Error refreshing device status for ID ${req.params.id}:`,
        error,
      );
      res.status(500).json({
        success: false,
  ...
  router.post("/clients/:id/traffic", async (req: Request, res: Response) => {
    try {
      ...
      const updated = await clientManagementService.updateDeviceTraffic(
        deviceId,
        trafficData,
      );
      ...
      console.error(
        `Error updating device traffic for ID ${req.params.id}:`,
        error,
      );
      res.status(500).json({
        success: false,
  ...
  router.get("/network-devices", async (req: Request, res: Response) => {
    try {
      const isIdentified = req.query.identified
      ...
      const devicesWithStatus =
        await clientManagementService.getNetworkDevices();

      // Merge the status information with the device data
      const mergedDevices = devices.map((device) => {
        const statusDevice = devicesWithStatus.find(
  ...
  router.get("/network-devices/:id", async (req: Request, res: Response) => {
    try {
      ...
      const history =
        await discoveryService.getDeviceDiscoveryHistory(deviceId);
  ...
  router.post(
    "/network-devices/:id/identify",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.id);
        const device =
          await deviceIdentificationService.identifyDevice(deviceId);
  ...
  router.post(
    "/discovery/dhcp/:deviceId",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.deviceId);
        const result =
          await schedulerService.runManualRouterDiscovery(deviceId);
        res.json(result);
      } catch (error) {
        console.error(
          `Error scanning DHCP from device ${req.params.deviceId}:`,
          error,
        );
  ...
  router.post("/discovery/interval", async (req: Request, res: Response) => {
    try {
      ...
      if (intervals.discoveryScanInterval) {
        result.discoveryScanInterval =
          schedulerService.setDiscoveryScanInterval(
            intervals.discoveryScanInterval,
          );
      }

      if (intervals.identificationScanInterval) {
        result.identificationScanInterval =
          schedulerService.setIdentificationScanInterval(
            intervals.identificationScanInterval,
          );
      }

      if (intervals.routerDiscoveryInterval) {
        result.routerDiscoveryInterval =
          schedulerService.setRouterDiscoveryInterval(
            intervals.routerDiscoveryInterval,
          );
      }
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid interval data", errors: error.errors });
      }
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!role) {
          return res.status(404).json({
            message: "Network device not found or could not be classified",
          });
        }
  ...
  router.post(
    "/network-devices/reclassify-all",
    async (_req: Request, res: Response) => {
      try {
  ...
  router.post("/network-scan", async (req: Request, res: Response) => {
    try {
      ...
      if (!networks && !autoDetect) {
        return res.status(400).json({
          message:
            "Phải cung cấp danh sách mạng (networks) hoặc bật tự động phát hiện (autoDetect)",
        });
      }
      ...
      res
        .status(500)
        .json({ message: "Lỗi khi quét mạng", error: error.message });
    }
  });
  ...
  router.post("/network-scan/ip", async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Lỗi khi quét địa chỉ IP", error: error.message });
    }
  });

  // Thu thập dữ liệu lưu lượng mạng cho thiết bị cụ thể
  router.post(
    "/network-devices/:id/collect-traffic",
    async (req: Request, res: Response) => {
      try {
        const deviceId = parseInt(req.params.id);
  ...
  router.post("/analyze-traffic/:id", async (req: Request, res: Response) => {
    try {
      ...
      const result = await logAnalyzerService.analyzeTrafficLogs(
        deviceId,
        options,
      );
  ...
  const collectAndBroadcastTraffic = async (deviceId: number) => {
    const result =
      await trafficCollectorService.collectTrafficByDeviceRole(deviceId);
