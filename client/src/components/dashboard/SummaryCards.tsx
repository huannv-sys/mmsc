...
const SummaryCards: React.FC<SummaryCardsProps> = ({ deviceId }) => {
  ...
  const { data: metrics } = useQuery<Metric[]>({
    queryKey: deviceId
      ? [`/api/devices/${deviceId}/metrics`, { limit: 1 }]
      : null,
    enabled: !!deviceId,
  });
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      ...
      <div className="bg-amber-700 rounded-lg shadow-md p-4 border border-amber-500">
        ...
        <div className="flex items-end">
          <div className="text-3xl font-bold text-white">
            {latestMetric
              ? `${Math.round(latestMetric.cpuUsage || 0)}%`
              : "N/A"}
          </div>
        </div>
        <div className="w-full bg-amber-800 rounded-full h-3 mt-3">
          <div
            ...
            style={{
              width: `${latestMetric ? Math.round(latestMetric.cpuUsage || 0) : 0}%`,
            }}
          ></div>
        </div>
      </div>
      ...
      <div className="bg-blue-700 rounded-lg shadow-md p-4 border border-blue-500">
        ...
        <div className="flex items-end">
          <div className="text-2xl font-bold text-white">
            {latestMetric
              ? `${formatBytes(latestMetric.memoryUsage)} / ${formatBytes(latestMetric.totalMemory)}`
              : "N/A"}
          </div>
        </div>
        <div className="w-full bg-blue-800 rounded-full h-3 mt-3">
          <div
            ...
            style={{
              width:
                latestMetric && latestMetric.totalMemory
                  ? `${Math.round(((latestMetric.memoryUsage || 0) / latestMetric.totalMemory) * 100)}%`
                  : "0%",
            }}
          ></div>
        </div>
      ...
      <div className="bg-emerald-700 rounded-lg shadow-md p-4 border border-emerald-500">
        ...
        <div className="flex items-end">
          <div className="text-2xl font-bold text-white">
            {latestMetric
              ? `${Math.round((latestMetric.uploadBandwidth || 0) + (latestMetric.downloadBandwidth || 0))} Mbps`
              : "N/A"}
          </div>
        </div>
      ...
      <div className="bg-red-700 rounded-lg shadow-md p-4 border border-red-500">
        ...
        <div className="flex items-end">
          <div className="text-3xl font-bold text-white">
            {latestMetric
              ? `${Math.round(latestMetric.temperature || 0)}°C`
              : "N/A"}
          </div>
        </div>
        <div className="w-full bg-red-800 rounded-full h-3 mt-3">
          <div
            ...
            style={{
              width: latestMetric
                ? `${Math.min(100, Math.round(((latestMetric.temperature || 0) / 80) * 100))}%`
                : "0%",
            }}
          ></div>
        </div>
