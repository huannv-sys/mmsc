...
const NetworkTrafficAdvanced: React.FC<NetworkTrafficAdvancedProps> = ({
  deviceId,
}) => {
  ...
  const interfacesEndpoint = deviceId
  ...
  const formatTrafficData = useCallback(() => {
    ...
    switch (selectedTimeFrame) {
      case "1h":
        timeFrameData = timeFrameData.filter(
          (m) => now - new Date(m.timestamp).getTime() <= hourMs,
        );
        break;
      case "6h":
        timeFrameData = timeFrameData.filter(
          (m) => now - new Date(m.timestamp).getTime() <= 6 * hourMs,
        );
        break;
      case "24h":
        timeFrameData = timeFrameData.filter(
          (m) => now - new Date(m.timestamp).getTime() <= dayMs,
        );
        break;
      case "7d":
        timeFrameData = timeFrameData.filter(
          (m) => now - new Date(m.timestamp).getTime() <= 7 * dayMs,
        );
        break;
      case "realtime":
      default:
        ...
        timeFrameData =
          timeFrameData.length <= 50 ? timeFrameData : timeFrameData.slice(-50);
    }
    ...
    const processedData = timeFrameData
      .map((metric, index) => {
        if (!metric) {
        ...
        if (index > 0 && timeFrameData[index - 1]) {
          ...
          const prevDownload =
            typeof prevMetric.downloadBandwidth === "number"
              ? prevMetric.downloadBandwidth
              : 0;
          const prevUpload =
            typeof prevMetric.uploadBandwidth === "number"
              ? prevMetric.uploadBandwidth
              : 0;
          ...
          if (timeDiff > 0) {
            ...
            const dlSpeed =
              Math.max(0, downloadBandwidth - prevDownload) / timeDiff;
            const ulSpeed =
              Math.max(0, uploadBandwidth - prevUpload) / timeDiff;
        ...
        const cpuUsage =
          typeof metric.cpuUsage === "number" ? metric.cpuUsage : 0;
        const memoryUsage =
          typeof metric.memoryUsage === "number" ? metric.memoryUsage : 0;
  ...
  const getCurrentTrafficStats = useCallback(() => {
    ...
    return {
      timestamp:
        typeof latestData.timestamp === "string"
  ...
  const calculateTotalBandwidthUsed = useCallback(() => {
    ...
    for (let i = 1; i < trafficData.length; i++) {
      ...
      if (timeDiff > 0 && timeDiff < 300) {
        // Loại bỏ khoảng thời gian bất thường
    ...
    if (validIntervals < 5 && interfaces && interfaces.length > 0) {
      ...
      totalBytes.download = Math.max(
        totalBytes.download,
        totalRx / (1024 * 1024 * 1024),
      );
      totalBytes.upload = Math.max(
        totalBytes.upload,
        totalTx / (1024 * 1024 * 1024),
      );
    }
    ...
    return (
      <div className="bg-gray-900 rounded-lg p-4 shadow-md flex flex-col items-center justify-center h-[600px]">
        <p className="text-gray-400 mb-4">
          {!hasValidMetrics && !hasValidInterfaces
            ? "Không có dữ liệu từ thiết bị"
            : !hasValidMetrics
              ? "Không có dữ liệu metrics từ thiết bị"
  ...
  const renderContent = () => {
    if (activeTab === "analysis") {
      return (
        <div className="p-4">
          {isLoadingLogAnalysis ? (
            ...
            <div className="bg-slate-950 rounded-lg p-4 shadow-md flex flex-col items-center justify-center h-[400px]">
              <p className="text-gray-400 mb-4">
                Chưa có dữ liệu phân tích. Nhấn nút bên dưới để bắt đầu phân
                tích log lưu lượng.
              </p>
              <Button
                onClick={analyzeTrafficLogs}
                ...
                <Card className="border-none bg-slate-950">
                  ...
                  <CardContent className="p-4 pt-0">
                    <div className="h-[200px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={Object.entries(logAnalysisData.topSources)
                            .map(([ip, bytes]) => ({
                              ip,
      ...
      const mainInterface =
        interfaces && interfaces.length > 0 ? interfaces[0] : null;

      // Get stats from interface data if available
      const stats = {
        txRate: currentStats.upload || 0,
        rxRate: currentStats.download || 0,
        txPacketRate: mainInterface?.txPackets
          ? Math.round(mainInterface.txPackets / 10)
          : 615, // p/s
        rxPacketRate: mainInterface?.rxPackets
          ? Math.round(mainInterface.rxPackets / 10)
          : 983, // p/s
        ...
        fpTxPacketRate: mainInterface?.txPackets
          ? Math.round((mainInterface.txPackets / 10) * 0.9)
          : 554, // p/s
        fpRxPacketRate: mainInterface?.rxPackets
          ? Math.round((mainInterface.rxPackets / 10) * 0.9)
          : 884, // p/s
                  ...
                  <table className="min-w-full bg-transparent text-sm">
                    <thead>
                      <tr className="border-b border-gray-800">
                        <th className="py-2 px-3 text-gray-400 text-left">
                          Metric
                        </th>
                        <th className="py-2 px-3 text-gray-400 text-left">
                          TX (Upload)
                    ...
                    <tbody>
                      <tr className="border-b border-gray-800">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {formatMbps(stats.txRate, 1)}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {formatMbps(stats.rxRate, 1)}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {formatMbps(stats.txRate + stats.rxRate, 1)}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        <td className="py-2 px-3 text-gray-300 font-medium">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {formatPacketRate(stats.txPacketRate)}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {formatPacketRate(stats.rxPacketRate)}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {formatPacketRate(
                            stats.txPacketRate + stats.rxPacketRate,
                          )}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {formatMbps(stats.fpTxRate, 1)}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {formatMbps(stats.fpRxRate, 1)}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {formatMbps(stats.fpTxRate + stats.fpRxRate, 1)}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        <td className="py-2 px-3 text-gray-300 font-medium">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {formatPacketRate(stats.fpTxPacketRate)}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {formatPacketRate(stats.fpRxPacketRate)}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {formatPacketRate(
                            stats.fpTxPacketRate + stats.fpRxPacketRate,
                          )}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        ...
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {formatBytes(stats.txBytes + stats.rxBytes)}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        <td className="py-2 px-3 text-gray-300 font-medium">
                        ...
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {(stats.txPackets + stats.rxPackets).toLocaleString()}
                        </td>
                      </tr>
                      <tr className="border-b border-gray-800">
                        <td className="py-2 px-3 text-gray-300 font-medium">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {stats.txDrops.toLocaleString()}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {stats.rxDrops.toLocaleString()}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {(stats.txDrops + stats.rxDrops).toLocaleString()}
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 text-gray-300 font-medium">
                        ...
                        <td className="py-2 px-3 text-orange-400 font-mono">
                          {stats.txErrors.toLocaleString()}
                        </td>
                        <td className="py-2 px-3 text-green-400 font-mono">
                          {stats.rxErrors.toLocaleString()}
                        </td>
                        <td className="py-2 px-3 text-blue-400 font-mono">
                          {(stats.txErrors + stats.rxErrors).toLocaleString()}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                          ...
                          <div className="bg-slate-900 rounded-md p-3 h-full">
                            ...
                            <div className="text-yellow-400 font-mono font-medium">
                              {totalBandwidth.upload > 0
                                ? (
                                    totalBandwidth.download /
                                    totalBandwidth.upload
                                  ).toFixed(1)
                                : "N/A"}
                            </div>
                          </div>
              ...
              <CardContent className="p-4 pt-0">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center justify-between bg-slate-900 rounded-md p-3">
                    ...
                    <Badge
                      variant={
                        Number(currentStats.traffic) > 15
                          ? "danger"
                  ...
                  <div className="flex items-center justify-between bg-slate-900 rounded-md p-3">
                    ...
                    <div className="text-sm font-mono">
                      {currentStats.upload > 0
                        ? (currentStats.download / currentStats.upload).toFixed(
                            1,
                          )
                        : "N/A"}
                    </div>
