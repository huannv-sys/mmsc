...
const CPUMemoryChart: React.FC<CPUMemoryChartProps> = ({ deviceId }) => {
    ...
    (metric: Metric) => {
      setRealTimeData((prevData) => {
        ...
        const newDataPoint = {
          ...
          memoryUsage: metric.totalMemory
            ? ((metric.memoryUsage || 0) / metric.totalMemory) * 100
            : 0,
        };

        // Giới hạn số lượng điểm
        const maxPoints =
          timeRange === "1H" ? 60 : timeRange === "24H" ? 24 : 7;
  ...
  const chartData =
    realTimeData.length > 0 ? realTimeData : formatChartData(metrics);
