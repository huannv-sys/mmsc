  ...
  const handleRefreshData = async () => {
    if (selectedDeviceId) {
      try {
        await apiRequest(
          "POST",
          `/api/devices/${selectedDeviceId}/refresh`,
          {},
        );
      ...
      <div className="flex items-center justify-between px-6 py-3">
        <div className="flex items-center">
          <button
            ...
            className="text-slate-300 hover:text-blue-400 focus:outline-none mr-4"
          >
            <svg
      ...
      <div className="px-6 py-2 bg-slate-700 border-t border-slate-600 flex items-center flex-wrap gap-2">
        ...
        {selectedDevice && (
          <div className="flex items-center space-x-1 ml-4">
            <span
              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedDevice.isOnline ? "bg-green-800 text-green-100" : "bg-red-800 text-red-100"}`}
            >
              <div
                className={`w-2 h-2 rounded-full mr-1 ${selectedDevice.isOnline ? "bg-green-400" : "bg-red-400"}`}
              ></div>
              {selectedDevice.isOnline ? "Online" : "Offline"}
            </span>
            <span className="text-xs text-slate-300">
              Uptime: {selectedDevice.uptime || "N/A"}
            </span>
          </div>
        )}
        <button
