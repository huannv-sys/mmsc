...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const classes = [baseClass, variantClass, sizeClass, activeClass, className]
      .filter(Boolean)
      .join(" ");
...
export const Breadcrumb: React.FC<BreadcrumbProps> & {
  ...
  return (
    <nav aria-label="breadcrumb">
      <ol className={`breadcrumb ${className}`}>{children}</ol>
    </nav>
  );
};
...
Breadcrumb.Item = ({ active = false, children, className = "", href }) => {
  const classes =
    `breadcrumb-item ${active ? "active" : ""} ${className}`.trim();
