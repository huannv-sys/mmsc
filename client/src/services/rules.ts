...
export async function getFirewallRules(deviceId: string): Promise<ApiResponse> {
  try {
    ...
    return {
      success: false,
      message: 'Không thể lấy thông tin quy tắc firewall. Vui lòng thử lại sau.'
    };
  }
}
...
export async function createFirewallRule(deviceId: string, ruleData: any): Promise<ApiResponse> {
  try {
    const response = await api.post(`/api/devices/${deviceId}/rules/firewall`, ruleData);
    return response.data;
  } catch (error) {
...
export async function updateRule(deviceId: string, ruleType: string, ruleId: string, ruleData: any): Promise<ApiResponse> {
  try {
    const response = await api.put(`/api/devices/${deviceId}/rules/${ruleType}/${ruleId}`, ruleData);
    return response.data;
  } catch (error) {
...
export async function deleteRule(deviceId: string, ruleType: string, ruleId: string): Promise<ApiResponse> {
  try {
    const response = await api.delete(`/api/devices/${deviceId}/rules/${ruleType}/${ruleId}`);
    return response.data;
  } catch (error) {
...
export async function toggleRuleStatus(deviceId: string, ruleType: string, ruleId: string, disabled: boolean): Promise<ApiResponse> {
  try {
    const response = await api.put(`/api/devices/${deviceId}/rules/${ruleType}/${ruleId}/toggle`, { disabled });
    return response.data;
  } catch (error) {
